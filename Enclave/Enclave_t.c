...
static sgx_status_t SGX_CDECL sgx_generate_ecc_key_pair(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_generate_ecc_key_pair_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_generate_ecc_key_pair_t* ms = SGX_CAST(ms_generate_ecc_key_pair_t*, pms);
	ms_generate_ecc_key_pair_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_generate_ecc_key_pair_t), ms, sizeof(ms_generate_ecc_key_pair_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	PublicKey* _tmp_enclave_public_key = __in_ms.ms_enclave_public_key;
	size_t _len_enclave_public_key = 64;
	PublicKey* _in_enclave_public_key = NULL;
	sgx_status_t _in_retval;

	CHECK_UNIQUE_POINTER(_tmp_enclave_public_key, _len_enclave_public_key);
	...
	if (_tmp_enclave_public_key != NULL && _len_enclave_public_key != 0) {
		if ((_in_enclave_public_key = (PublicKey*)malloc(_len_enclave_public_key)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_enclave_public_key, 0, _len_enclave_public_key);
	}
	_in_retval = generate_ecc_key_pair(_in_enclave_public_key, __in_ms.ms_key_size);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
	if (_in_enclave_public_key) {
		if (memcpy_verw_s(_tmp_enclave_public_key, _len_enclave_public_key, _in_enclave_public_key, _len_enclave_public_key)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	if (_in_enclave_public_key) free(_in_enclave_public_key);
	return status;
}

static sgx_status_t SGX_CDECL sgx_teste(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_teste_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_teste_t* ms = SGX_CAST(ms_teste_t*, pms);
	ms_teste_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_teste_t), ms, sizeof(ms_teste_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	const unsigned char* _tmp_msg = __in_ms.ms_msg;
	size_t _tmp_msg_size = __in_ms.ms_msg_size;
	size_t _len_msg = _tmp_msg_size;
	unsigned char* _in_msg = NULL;
	Signature* _tmp_enclave_signature = __in_ms.ms_enclave_signature;
	size_t _len_enclave_signature = 64;
	Signature* _in_enclave_signature = NULL;
	sgx_status_t _in_retval;

	CHECK_UNIQUE_POINTER(_tmp_msg, _len_msg);
	CHECK_UNIQUE_POINTER(_tmp_enclave_signature, _len_enclave_signature);
	...
	if (_tmp_enclave_signature != NULL && _len_enclave_signature != 0) {
		if ((_in_enclave_signature = (Signature*)malloc(_len_enclave_signature)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_enclave_signature, 0, _len_enclave_signature);
	}
	_in_retval = teste((const unsigned char*)_in_msg, _in_enclave_signature, _tmp_msg_size);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
	if (_in_enclave_signature) {
		if (memcpy_verw_s(_tmp_enclave_signature, _len_enclave_signature, _in_enclave_signature, _len_enclave_signature)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	...
	if (_in_enclave_signature) free(_in_enclave_signature);
	return status;
}
...
SGX_EXTERNC const struct {
	...
	6,
	{
		...
		{(void*)(uintptr_t)sgx_generate_ecc_key_pair, 0, 0},
		{(void*)(uintptr_t)sgx_teste, 0, 0},
	}
};
